...
- **任务模式 `toolWhitelist` 是生效的工具交集过滤器，对当前内置模式退化为恒等。** `star-domain.ts` 有 16 个模式（项目统筹/探索新方案/维护结构/执行任务/评估方案/检查前提/跨模块分析/优化提示词/整理代码/核对运行结果/复现并验证/跟进长期任务/日常开发/检查界面与交付/精简冗余/使用最小工具集）。worker 创建时 `allowedTools = profile.allowedTools ∩ domain.toolWhitelist`（`work-order.ts:toolsForAuthority`）。当前 16 个模式的白名单**逐字相同且是全集**（2026-07-15 起含 `browser_debug` + `computer_use`） → 对内置模式交集退化为恒等（no-op），模式间行为差异只来自 `systemPromptSuffix`/`volatileBlock`/`courageThreshold`/`decisionStyle`。但机制本身**真实生效**，三处会咬人：①profile 若含白名单外工具（`browser` 等），设了 authority 就被静默削掉；②authority 拼错/模式未加载 → **fail-closed 返回 `[]` deny-all**（有意护栏，勿改成回退 profile 全集）；③自定义模式（card frontmatter）的 `toolWhitelist` 完全生效，会真实削减工具。改它前先想清楚命中的是哪种场景。
...
## Star Lore (historical)

历史命名与星图叙事（伙伴星定义）见 [star.md](./star.md) —— 这是**叙事存档，不是现行使用说明**。产品对外的现行叫法是「任务模式」：命令 `/task-mode`（`/domain` 为旧别名仍可用），实现见 `src/agent/star-domain.ts` + `src/agent/star-domain-data.ts`，16 个模式的显示名与说明见 README「任务模式」。
